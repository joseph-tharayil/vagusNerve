...
def Recruitment(current,diameters, fascIdx):
    ...
    titrationFactorsMeff = sortTitrationSpace(pd.read_excel('/gpfs/bbp.cscs.ch/project/proj85/scratch/vagusNerve/Data/Titration_SENN_HighConductivity.xlsx',index_col=0)).iloc[-1].values
    
    titrationFactorsUaff = sortTitrationSpace(pd.read_excel('/gpfs/bbp.cscs.ch/project/proj85/scratch/vagusNerve/Data/Titration_Sundt.xlsx',index_col=0)).iloc[-1].values
...
def main(outputfolder,distanceIdx):
    ...
    current = np.linspace(100,500,5)/173
