...
def Recruitment(current,variance,diameters, fascIdx):
    ...
    for j in [0,1]: # Myelinated and unmyelinated, respectively
        ...
        titrationFac += np.random.normal(0,variance*np.mean(titrationFac),len(titrationFac))
...
def Scaling(d,fiberType): # Diameter dependent scaling of signal
    ...
    if fiberType == 0: # Myelinated fiber
        ...
        deff = 0.7 * d # Calculates internal diameter from external diameter
