...
def FitPhiShape(fascIdx,distance):
    ...
    phi = pd.read_excel('/gpfs/bbp.cscs.ch/project/proj85/scratch/vagusNerve/Data/PhiConductivity_Monopolar_Second_Corrected/'+str(fascIdx)+'_BetterConductivity.xlsx')
...
def main(outputfolder,distanceIdx):
    ...
    distances = np.array([0.06,.05,0.01]) + 3 * 3e-3 # Stimulus-recording distance, in m
...
#     current = np.linspace(100,500,5)/180 # Currents from 100 to 1000 uA, scaled by the current in the S4L titration simulation
    ...
    recordingCurrent = 409e-6 # Current in the S4L recording simulation
