...
def UeffProb(d, ueffProb):
    ...
    return ueffProb * prob(d,ueffvals,False)
...
def Recruitment(current,diameters, fascIdx):
    ...
    titrationFactorsMeff = sortTitrationSpace(pd.read_excel('/gpfs/bbp.cscs.ch/project/proj85/scratch/vagusNerve/Data/TitrationGoodConductivity_Standoff.xlsx',index_col=0)).iloc[-1].values
    ####


    titrationFactors = [titrationFactorsMeff, titrationFactorsMeff]
...
##############
    ...
    for j, d in enumerate(diameters):
        ...
        myelinated.append(interp(current*(d/d0Myelinated)**2))
...
def Scaling(d,fiberType): # Diameter dependent scaling of signal
    ...
    if fiberType == 0: # Myelinated fiber
        ...
        deff = 0.32 * d # Calculates internal diameter from external diameter
...
##### Weight is given by the product of the recruitment curve and the diameter probability curve
    ...
    np.save(outputfolder+'/'+str(0)+'/fascicles'+'/probs'+str(fascIdx)+'.npy',[maffProb,meffProb])
    np.save(outputfolder+'/'+str(0)+'/fascicles'+'/probDist'+str(fascIdx)+'.npy',[MaffProb(d,maffProb),MeffProb(d,meffProb)])
...
def main(outputfolder,distanceIdx):
    ...
    distances = [0.07,.05] # Stimulus-recording distance, in m
...
#     current = np.linspace(100,500,5)/28
...
#     current = np.linspace(100,500,5)/180 # Currents from 100 to 1000 uA, scaled by the current in the S4L titration simulation
    ...
    recordingCurrent = 577e-6 # Current in the S4L recording simulation
