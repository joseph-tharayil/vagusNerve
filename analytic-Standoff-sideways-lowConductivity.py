...
def Recruitment(current,diameters, fascIdx):
    ...
    titrationFactorsMeff = sortTitrationSpace(pd.read_excel('/gpfs/bbp.cscs.ch/project/proj85/scratch/vagusNerve/Data/Titration_Rat_Sideways_LowConductivity.xlsx',index_col=0)).iloc[-1].values
    
    titrationFactorsUaff = sortTitrationSpace(pd.read_excel('/gpfs/bbp.cscs.ch/project/proj85/scratch/vagusNerve/Data/Titration_Sundt_Sideways_LowConductivity.xlsx',index_col=0)).iloc[-1].values
...
def FitPhiShape(fascIdx,distance):
    ...
    phi = pd.read_excel('/gpfs/bbp.cscs.ch/project/proj85/scratch/vagusNerve/Data/PhiConductivity_LowConductivity_20240217/'+str(fascIdx)+'_BetterConductivity.xlsx')
...
def main(outputfolder,distanceIdx):
    ...
    current = np.linspace(100,500,5)/27.4786

    
    recordingCurrent = 490.646e-6 # Current in the S4L recording simulation
