...
def Scaling(d,fiberType): # Diameter dependent scaling of signal
    ...
    if fiberType == 0: # Myelinated fiber
        ...
        deff = d# 0.32 * d # Calculates internal diameter from external diameter
...
def editPhiShape(phi,distance):
   ...
   ######## 
    ...
    if np.any(phiShapeEmpirical[:np.argmax(phiShapeEmpirical)]<0): # If the potential is negative at the end of the fiber, sets the potential to 0
        ...
        first = np.where(np.abs(np.diff(phiShapeEmpirical))>1e-6)[0][0] # Based on derivative of function, selects point after whcih not to change values
        

        ### Linearizes potential before this point, up until it reaches 0 
        firsta = np.where(phiShapeEmpirical[first]-1e-6*np.arange(first)<0)[0][0]
        firsta = first-firsta

        phiShapeEmpirical[firsta:first] = 1e-6*np.arange(first-firsta)
        #######
    ...
    if np.any(phiShapeEmpirical[np.argmin(phiShapeEmpirical):]>0):
        ...
        last = np.where(np.abs(np.diff(phiShapeEmpirical))>1e-6)[0][-1]
        lasta = np.where(phiShapeEmpirical[last]+1e-6*np.arange(len(phiShapeEmpirical)-last)>0)[0][0]
        lasta += last

        phiShapeEmpirical[last:lasta] = 1e-6* np.arange(lasta-last)+ phiShapeEmpirical[last]
...
def FitPhiShape(fascIdx,distance):
    ...
    phi = pd.read_excel('/gpfs/bbp.cscs.ch/project/proj85/scratch/vagusNerve/Data/PhiConductivity_Small_Otherside_20240215/'+str(fascIdx)+'_BetterConductivity.xlsx')
...
def main(outputfolder,distanceIdx):
    ...
    current = np.linspace(500,500,1)/28.6

    recordingCurrent = 273e-6 # Current in the S4L recording simulation
