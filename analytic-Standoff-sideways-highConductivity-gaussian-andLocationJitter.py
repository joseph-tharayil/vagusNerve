...
def Recruitment(current,variance,diameters, fascIdx):
    ...
    for j in [0,1]: # Myelinated and unmyelinated, respectively
        ...
        titrationFac += np.random.normal(0,variance*titrationFac.astype(float))
...
def PhiShape(velocity,t,function,generator):
...
#     sos = butter(1, 20000, 'lp', fs=83333, output='sos') # We apply a low-pass filter to avoid numerical issues
    ...
    for i, v in enumerate(velocity): # Iterates through fibers
        ...
        x = t*v+generator.uniform(-.001,.001) ### Defines interpolation points from time vector and fiber velocity
...
def PhiWeight(d, current, variance, fascIdx, fascTypes):
    
    phiWeight = [ [[],[]], [[],[]] ]
    
    recruitment = Recruitment(current,variance,d,fascIdx)
...
def getPhiWeight(d, current,variance,fascIdx,fascTypes):
    ...
    for c in current:
        ...
        for v in variance:
            ...
            p, rec = PhiWeight(d,c,v,fascIdx,fascTypes)
...
def main(outputfolder,distanceIdx):
    ...
    phiWeight0, phiWeight1, phiWeight2, phiWeight3 = getPhiWeight(d,current,variance,fascIdx, fascTypes) # Scaling factor for each diameter
    ...
    phiShape0 = PhiShape(velocityList[0],tphi,phiFunc,latencyGenerator)
    
    phiShape1 = PhiShape(velocityList[1],tphi,phiFunc,latencyGenerator)
