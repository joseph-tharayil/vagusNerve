...
for ent in ents['Fascicles'].Entities:
	...
	splinesMeff = s4l.model.CreateGroup('SplinesMeff_'+ ent.Name)

	splinesList = [splinesMeff]
...
for i, fold in enumerate(ents['Axons_Folder_2'].Entities):
	...
	for j, ent in enumerate(fold.Entities):
		...
		rat_settings = model.RatNeuronProperties()
		components = [ent]

		rat_settings.AxonDiameter = r
		nrn = model.CreateAxonNeurons(components, rat_settings)
